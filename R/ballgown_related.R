...
BallgownDraw <- function(sample.pattern = "NO_DATA", covariate = "NO_DATA") {
  if (sample.pattern == "NO_DATA" || covariate == "NO_DATA") {
    ...
    sample.table <- as.data.frame(table(pheno_data.arrange$sex))
    if (length(row.names(sample.table)) != 2) {
      ...
      for(i in 1:length(row.names(sample.table))) {
        ...
        for(j in 1:sample.table$Freq[i]){
          print(j)
          a <- paste0("FPKM.", sample.names[6*(i-1)+j])
          results_transcripts[[sample.names.with.covariate[6*(i-1)+j]]] <- fpkm[[a]]
          columns.to.mean <- append(columns.to.mean, sample.names.with.covariate[6*(i-1)+j])
        }
        print(columns.to.mean)
        #rowMeans(results_transcripts[columns.to.mean])
      ...
      results_transcripts[["all.mean"]] <- rowMeans(results_transcripts[all.mean])


      ## Ma plot
      ggplot(results_transcripts, aes(log2( all.mean), log2FC, colour = qval<0.05)) +
      ...
      topT <- as.data.frame(results_transcripts)
      ...
      for(i in 1:length(row.names(sample.table))){
        for(j in 1:6){
          plot.column.number <- 8+j+(sample.table$Freq[i]+1)*(i-1)
          print(8+j+(sample.table$Freq[i]+1)*(i-1))
