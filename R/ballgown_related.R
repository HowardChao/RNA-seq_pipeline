...
#' @export
BallgownPreprocess <- function(sample.pattern = "NO_DATA", covariate = "NO_DATA") {
  if (sample.pattern == "NO_DATA" || covariate == "NO_DATA") {
    ...
    if (length(row.names(sample.table)) == 2) {
      dir.create(paste0(pkg.global.path.prefix$data_path, "gene_data/differential_results/"))
      ...
      write.csv(results_transcripts, paste0(pkg.global.path.prefix$data_path, "gene_data/differential_results/DEG_result.csv"), row.names=FALSE)
