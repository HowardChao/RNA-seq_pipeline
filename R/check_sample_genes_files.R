    ...
    if (gene_name == "NO_DATA"){
      ...
      if (fastq.gz.files.number != 0){
        for (i in fastq.gz.files){
          cat(c("(O) :", paste0("'", getwd(), '/raw_fastq.gz/', i, "'"), "is exit\n"))
        }
        cat(c("Total:", fastq.gz.files.number, "files\n\n"))
      }else {
        cat(c("(X) :", paste0('\'',getwd(), '/raw_fastq.gz/XXX_*.fastq.gz\''), "is not exit\n"))
      ...
      if (sam.files.number != 0){
        for (i in sam.files){
          cat(c("(O) :", paste0("'", getwd(), '/raw_sam/', i, "'"), "is exit\n"))
        }
        cat(c("Total:", sam.files.number, "files\n\n"))
      }else {
        cat(c("(X) :", paste0('\'',getwd(), '/raw_sam/XXX.sam\''), "is not exit\n"))
      ...
      if (bam.files.number != 0){
        for (i in bam.files){
          cat(c("(O) :", paste0("'", getwd(), '/raw_bam/', i, "'"), "is exit\n"))
        }
        cat(c("Total:", bam.files.number, "files\n\n"))
      }else {
        cat(c("(X) :", paste0('\'',getwd(), '/raw_sam/XXX.bam\''), "is not exit\n"))
      ...
      if (gtf.files.number != 0){
        for (i in gtf.files){
          cat(c("(O) :", paste0("'", getwd(), '/raw_gtf/', i, "'"), "is exit\n"))
        }
        cat(c("Total:", gtf.files.number, "files\n\n"))
      }else {
        cat(c("(X) :", paste0('\'',getwd(), '/raw_gtf/XXX.gtf\''), "is not exit\n"))
      ...
      if (isTRUE(stringtie_merged.gtf.file)) {
        cat(c("(O) :", paste0("'",getwd(), "/merged/stringtie_merged.gtf", "'"), "is exit\n\n"))
      } else {
        cat(c("(X) :", paste0("'",getwd(), "/merged/stringtie_merged.gtf", "'"), "is not exit\n"))
      ...
      if (ballgown.dirs.number != 0){
        ...
        cat(c("(X) :", paste0('\'',getwd(), '/ballgown/XXX/'), "is not exit\n"))
